...
├── gpuFunctions.cu                 # CUDA kernel implementations
├── gpuFunctions.cuh                # CUDA function declarations
├── Header.cuh                      # Additional CUDA headers
├── particles.cpp                   # Main particle system implementation
├── particles.h                     # Particle class definition
...
2. **Kernel Execution**: Two main CUDA kernels compute:
   - `calculateNewPositionGPU`: Calculates new particle positions using Verlet integration
   - `calculateVelocitiesGPU`: Computes particle velocities from position differences
3. **Force Calculation**: 
...
1. **Block Size**: Adjust CUDA block size in `launchKernel*` functions (currently 256 threads)
